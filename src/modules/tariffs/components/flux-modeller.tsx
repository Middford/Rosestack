  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-text-tertiary ml-2">Standard Octopus Flux — 365-day simulation — you control the battery</span>
      </div>
      ...
      <Card className="p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            <label className={labelClass}>Flux Rates (imp / exp)</label>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <SimpleStatCard
          ...
          value={`£${Math.round(result.annual.peakExport / 100).toLocaleString()}`}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {result.monthly.map(m => (
                <tr key={m.month} className="border-b border-border/50 hover:bg-bg-secondary/50">
                  ...
                  <td className="py-2 px-3 text-right">£{(m.peakExpRev / 100).toFixed(0)}</td>
                  <td className="py-2 px-3 text-right text-green-400">£{((m.peakSelfUse + m.eveSelfUse + m.nightSelfUse) / 100).toFixed(0)}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-border">
                ...
                <td className="py-2 px-3 text-right font-semibold">£{(result.annual.peakExport / 100).toFixed(0)}</td>
                <td className="py-2 px-3 text-right text-green-400 font-semibold">£{((result.annual.peakSelfUse + result.annual.eveSelfUse + result.annual.nightSelfUse) / 100).toFixed(0)}</td>
      ...
      <div className="text-xs text-text-tertiary bg-bg-secondary/50 rounded-lg p-4 border border-border/50">
        <p><strong className="text-violet-400">Flux Model Assumptions:</strong> YOU control the battery — no Kraken, no mandated reserve (5% safety floor only).
        Off-peak charge at 17.90p, reduced by predicted solar. Day-rate top-up only if profitable (26.80p charge vs 30.68p export).
        Peak export: {exportLimitKw}kW × 3hrs = {result.system.maxPeakExportKwh}kWh/day at 30.68p (import = export parity at peak).
        House on battery 24/7. 5% discharge floor.
